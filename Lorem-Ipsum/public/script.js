...
let para_count = document.getElementById("input-count");
para_count.focus();
para_count.placeholder = 5;
para_count.addEventListener("keyup", function(event) {
    if(event.keyCode===13) {
        generateLoremIpsum();
    }
});
let generateLoremIpsum = () => {
    removeChildren();
    if(para_count.value==="" || para_count.value>9) {
        textContainer.append(createTextElement(Math.floor(Math.random()*9)));
    }
    else {
        for(let i=0 ; i<para_count.value ; i++) {
            textContainer.append(createTextElement(i));
        }
    }
}
let createTextElement = (index) => {
